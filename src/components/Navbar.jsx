...
const navItems = [
  {
    label: "About Us",
    items: [
      { label: "About Vashishth Academy", path: "/about" },
  ...
  },
  {
    label: "Our Courses",
    items: [
      { label: "Coaching for IAS Exam", path: "/coaching/ias" },
      { label: "Coaching for PCS Exam", path: "/coaching/pcs" },
      { label: "Coaching for NDA Exam", path: "/coaching/nda" },
      { label: "Coaching for CDS Exam", path: "/coaching/cds" },
      { label: "Coaching for SSC Exam", path: "/coaching/ssc" },
      { label: "Interview Discussions", path: "/interview-discussions" },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Reference Books", path: "/reference-books" },
    ],
  },
  {
    label: "Daily Updates",
    items: [
      ...
      { label: "Monthly Magazines", path: "/monthly-magazines" },
    ],
  },
  {
    label: "Media",
    items: [
      ...
      { label: "Math Topic Videos", path: "/math-videos" },
    ],
  },
  { label: "Announcement", items: [], direct: true, path: "/announcement" },
];
  ...
  return (
    <nav style={{ background: "#fff", position: "sticky", top: 0, zIndex: 500, fontFamily: "'Hind',sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "0 16px", height: 64, gap: 8 }}>
        ...
        <div className="desktop-actions" style={{ display: "flex", gap: 8, flexShrink: 0, marginLeft: "auto" }}>
          ...
          <button
            ...
            style={{ background: C.gold, color: C.navy, fontSize: 10, fontWeight: 700, letterSpacing: ".06em", padding: "7px 12px", borderRadius: 4, textTransform: "uppercase", cursor: "pointer", border: "none", whiteSpace: "nowrap", transition: "all .2s" }}
            onMouseEnter={e => { e.currentTarget.style.background = "#fb8c00"; e.currentTarget.style.color = "#fff"; }}
      ...
      <div
        ...
        style={{ display: "none", background: "#fff", borderTop: "1px solid #eee", overflow: "hidden", maxHeight: menuOpen ? 800 : 0, transition: "max-height .35s ease" }}>
          ...
          <div key={idx} style={{ borderBottom: "1px solid #f0f4f9" }}>
            {item.direct ? (
              ...
              <>
                ...
                {item.items.length > 0 && (
                  <div style={{ maxHeight: openSub === idx ? 500 : 0, overflow: "hidden", transition: "max-height .25s ease", background: "#f8fbff" }}>
                    {item.items.map((sub, si) => (
                      <Link
                        key={si}
                        to={sub.path}
        ...
        <div style={{ display: "flex", gap: 10, padding: "14px 20px" }}>
          <button onClick={() => { onContact(); closeMenu(); }} style={{ flex: 1, padding: 9, fontSize: 11, fontWeight: 700, background: C.navy, color: "#fff", borderRadius: 4, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: ".05em" }}>
            Contact Us
          </button>
          <button
            onClick={() => { window.open("/vashishth-brochure.pdf", "_blank"); closeMenu(); }}
            style={{ flex: 1, padding: 9, fontSize: 11, fontWeight: 700, background: C.gold, color: C.navy, borderRadius: 4, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: ".05em" }}>
            Brochures
          </button>
        </div>
      </div>
